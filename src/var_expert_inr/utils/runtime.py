...
def get_thread_limits(default_threads: int = 64) -> tuple[int, int]:
    intra_threads = _parse_positive_int(
        os.environ.get("VAR_EXPERT_INR_NUM_THREADS"),
        default_threads,
    )
    interop_threads = _parse_positive_int(
        os.environ.get("VAR_EXPERT_INR_NUM_INTEROP_THREADS"),
        intra_threads,
    )
...
def apply_runtime_thread_limits(default_threads: int = 64) -> tuple[int, int]:
    global _THREADPOOL_LIMITER
    global _RUNTIME_LIMITS_APPLIED

    intra_threads, interop_threads = configure_thread_env(default_threads=default_threads)
...
# Apply BLAS/OpenMP limits before numpy / torch are imported elsewhere.

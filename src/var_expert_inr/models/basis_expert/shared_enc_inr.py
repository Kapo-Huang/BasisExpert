...
class SharedEncINR(nn.Module):
    ...
    def forward(
        ...
        *,
        ...
        enc_feat = self.encoder(self.pos_enc(coords))
        preds = {}
...
def build_shared_enc_inr_from_config(cfg: Dict, view_specs: Dict[str, int]) -> SharedEncINR:
    if {"enc_feature_dim", "view_embed_dim", "enc_hidden_dim", "decoder_hidden_dim"}.issubset(cfg):
        decoder_feature_dim = int(cfg["decoder_feature_dim"])
        head_hidden_raw = cfg.get("head_hidden_dim")
        enc_feature_dim = int(cfg["enc_feature_dim"])
        view_embed_dim = int(cfg["view_embed_dim"])
        enc_hidden_dim = int(cfg["enc_hidden_dim"])
    ...
    return SharedEncINR(
        ...
        enc_num_frequencies=enc_num_frequencies,
        ...
        enc_first_omega_0=enc_first_omega_0,
        enc_hidden_omega_0=enc_hidden_omega_0,

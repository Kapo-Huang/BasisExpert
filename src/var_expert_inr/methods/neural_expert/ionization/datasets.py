...
def _compute_input_stats(volume_shape):
    ...
    means = np.array([_mean(x_size), _mean(y_size), _mean(z_size), _mean(t_size)], dtype=np.float32)
    stds = np.sqrt(
        np.maximum(
            np.array([_var(x_size), _var(y_size), _var(z_size), _var(t_size)], dtype=np.float64) * correction,
            1.0e-12,
        )
    ).astype(np.float32)
...
class IonizationINRDataset(Dataset):
    def __init__(self, cfg, file_id=None):
        ...
        self.attr_name = str(data_cfg.get("attr_name") or file_id or Path(data_cfg["target_path"]).stem.replace("target_", ""))
...
def build_dataset(cfg, file_id, training=True):
    return IonizationINRDataset(cfg, file_id)

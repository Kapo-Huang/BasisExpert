...
class BalancingLossMoE(nn.Module):
    def __init__(self, n_experts, sample_bias_correction):
        ...
        self.sample_bias_correction = bool(sample_bias_correction)
    ...
    def compute_loss(self, q, gt_segment, *args, **kwargs):
        ...
        if self.seg_type == "both":
            return F.cross_entropy(q, gt_segment.long(), label_smoothing=0.01) + 0.1 * F.binary_cross_entropy_with_logits(
                q,
                torch.ones_like(q.squeeze()) / self.n_experts,
            )
...
class BaseReconstructionLoss(nn.Module):
    def __init__(self, cfg, model_name, in_dim=4, model=None, n_experts=1):
        ...
        self.model = model
        self.gradient_comp = train_utils.experts_gradient if moe_indicator else train_utils.gradient
        self.weight_dict = {}
        ...
        self.segmentation_loss = SegmentationLoss(
            self.required_loss_dict["segmentation"],
            n_experts,
            cfg.get("segmentation_type", "ce"),
        )
    ...
    def forward(self, output_pred, data=None, dataset=None):
        if data is None:
            raise ValueError(f"{self.__class__.__name__} requires a data batch")
        ...
        if gt_vals is None:
            raise ValueError(f"{self.__class__.__name__} expected 'nonmnfld_val' when segmentation_mode=False")
        ...
        load_term = (
            self.load_loss(output_pred.get("importance", None), output_pred.get("load", None), q)
            if q is not None
            else gt_vals.new_zeros(())
        )
        loss = (

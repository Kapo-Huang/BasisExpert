...
from ..models.modules import DummyModule, FullyConnectedNN, InputEncoder, ParallelFullyConnectedNN, tSoftMax
...
class INR_MoE(nn.Module):
    def __init__(self, cfg_all):
        ...
        self.n_experts = cfg["n_experts"]
        self.share_encoder = bool(cfg["shared_encoder"])
        ...
        if not self.share_encoder:
            ...
            manager_first_layer_dim = decoder_first_layer_dim
        if cfg["manager_type"] == "pointnet":
            manager_first_layer_dim += 1024
    ...
    def forward(self, non_mnfld_pnts, mnfld_pnts=None, **kwargs):
        ...
        if mnfld_pnts is not None:
            ...
            manifold_pnts_pred = self.decoder(encoded_mnfld_pnts_experts).squeeze(-1)
            manager_input = (
                self.manager_input_encoding_module(mnfld_pnts, **kwargs)
                if not self.share_encoder
                else encoded_mnfld_pnts_experts
            )
            manager_input = self.manager_conditioner(encoded_mnfld_pnts_experts, manager_input, **kwargs)
            mnfld_q, mnfld_selected_expert_idx, mnfld_raw_q = self.manager_net(manager_input.view(-1, manager_input.shape[-1]))
            mnfld_q = mnfld_q.T.unsqueeze(0)
        ...
        nonmanifold_pnts_pred = self.decoder(encoded_non_mnfld_pnts_experts)
        encoded_non_mnfld_pnts_manager = (
            self.manager_input_encoding_module(non_mnfld_pnts, **kwargs)
            if not self.share_encoder
            else encoded_non_mnfld_pnts_experts
        )
        ...
        nonmnfld_q = mout_nonmnfld_q.T.unsqueeze(0)

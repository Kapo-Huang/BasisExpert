...
def _load_or_compute_stats(
    ...
    if stats_path is not None and not stats_path.exists() and (load_input_stats or load_target_stats):
        ...
        np.savez(str(stats_path), x_mean=x_mean, x_std=x_std, y_mean=y_mean, y_std=y_std)
    return (
...
class CoordinateKMeansAssignments:
    def __init__(self, source: np.ndarray, n_experts: int, x_mean: np.ndarray, x_std: np.ndarray, cfg: _AssignmentConfig):
        ...
        if self.cache_path is not None and self.cache_path.exists():
            try:
                ...
                if centers.shape != (self.n_experts, source.shape[1]):
                    raise ValueError("Cached centers shape mismatch")
                ...
                if cached_normalize != self.normalize_features:
                    raise ValueError("Cached centers normalize_features mismatch")
                return centers
            except Exception:
                pass
...
class MeshAttributeDataset(Dataset):
    def __init__(self, cfg: dict[str, Any], file_id: str | None = None):
        ...
        if self.dataset_name not in {"katrina", "linkage_p", "linkage_c"}:
    ...
    def __getitem__(self, idx: int) -> dict[str, torch.Tensor]:
        ...
        if self.segmentation_mode:
            ...
            return {"nonmnfld_points": coords_t, "nonmnfld_segments_gt": torch.from_numpy(segments)}

        values = _ensure_2d(np.asarray(self.target[sample_idx], dtype=np.float32))
        if self.normalize_targets:
            values = (values - self.y_mean_np) / self.y_std_np
        return {"nonmnfld_points": coords_t, "nonmnfld_val": torch.from_numpy(values.astype(np.float32, copy=False))}


def build_dataset(cfg, file_id, training=True):
    return MeshAttributeDataset(cfg, file_id)
...
def build_dataloader(cfg, file_id, training=True):
    ...
    dataloader = DataLoader(
        dataset=dataset,
        num_workers=int(cfg[split].get("num_workers", 0)),
        pin_memory=False,

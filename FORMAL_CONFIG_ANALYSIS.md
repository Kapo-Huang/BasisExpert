...
| **APMGSRN** | 主优化器 `5e-5`；Adam + VarExpert progress-StepLR | 16,000/iteration；逐 timestep | 总预算 14.400B vs 14.4072B，差 0.05% | 按训练进度对齐 | 无 epoch；将 VarExpert 的 `40/600` 衰减节奏映射为 Ionization 每 600、Combustion 每 30 iterations；辅助 transform optimizer 保持 `0.05×lr` |
| **InstantVNR** | 两个规则数据集均 `5e-5`；Adam + StepLR(40, 0.92) | `16,000 × 1,500`，累积 4 个 batch | 24.0M/epoch；总计 14.4B | 优化策略一致 | lr、betas、eps、weight decay 和 epoch scheduler 与 VarExpert 对齐 |
...
| **APMGSRN** | architecture、历史 RD smoke | Ionization 保留 balanced，Combustion 使用已有 Size082；主优化器改为 VarExpert 策略，辅助 transform optimizer 保留方法专用 `0.05×` 学习率 | 协议对齐/结构中等 | 两个数据集各用一个属性确认映射后的 StepLR 收敛速度 |
| **InstantVNR** | v5 optimizer stability | size profile 保留；正式 optimizer 从 v5 优胜的 `1e-3` 改为 VarExpert `5e-5 + StepLR` | 协议对齐，性能证据弱 | 两个数据集各做短程确认，重点检查早期收敛不足 |

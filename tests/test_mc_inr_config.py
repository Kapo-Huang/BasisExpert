...
class MCINRConfigTestCase(unittest.TestCase):
    ...
    def test_loads_new_defaults(self):
        ...
        self.assertAlmostEqual(loaded.training.meta_support_ratio, 0.3)
        self.assertAlmostEqual(loaded.training.meta_outer_lr, 5.0e-5)
        self.assertTrue(loaded.training.cluster_aware_batches)

    def test_legacy_meta_fields_fallback(self):
        config = self._base_config()
        config["training"].update(
            {
                "epochs": 7,
                "meta_sampling_ratio": 0.4,
                "lr": 1.0e-3,
            }
        )
        ...
        self.assertAlmostEqual(loaded.training.meta_sampling_ratio, 0.4)
        self.assertAlmostEqual(loaded.training.meta_support_ratio, 0.4)
        self.assertAlmostEqual(loaded.training.meta_outer_lr, 1.0e-3)

    def test_explicit_meta_fields_override_legacy_aliases(self):
        config = self._base_config()
        config["training"].update(
            {
                "epochs": 9,
        ...
        self.assertAlmostEqual(loaded.training.meta_support_ratio, 0.25)

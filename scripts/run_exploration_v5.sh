...
CONFIG_ROOT="${REPO_ROOT}/configs_exploration_v5"
...
if [[ "${#FV_CONFIGS[@]}" -ne 24 || "${#INSTANT_CONFIGS[@]}" -ne 18 ]]; then
    printf 'Unexpected v5 stage counts: fv_srn=%d instant_vnr=%d. Regenerate with scripts/generate_exploration_v5_configs.py.\n' \
...
if [[ "${DRY_RUN}" != "1" ]]; then
    ...
    if ! conda run --no-capture-output -n "${CONDA_ENV}" python \
        "${SCRIPT_DIR}/summarize_exploration_v5.py" "${summary_args[@]}"; then

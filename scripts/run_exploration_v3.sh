...
CONFIG_ROOT="${REPO_ROOT}/configs_exploration_v3"
...
for size in "${SIZES[@]}"; do
    collect_group NeuralExpert "${size}" manager
done
...
for size in "${SIZES[@]}"; do
    collect_group NeuralExpert "${size}" main
done
...
if [[ "${total}" -ne "${EXPECTED_TOTAL}" ]]; then
    printf 'Expected %d exploration-v3 configs, found %d. Regenerate with scripts/generate_exploration_v3_configs.py.\n' \
...
if [[ "${DRY_RUN}" != "1" ]]; then
    ...
    if ! conda run --no-capture-output -n "${CONDA_ENV}" python \
        "${SCRIPT_DIR}/summarize_exploration_v3.py" "${summary_args[@]}"; then

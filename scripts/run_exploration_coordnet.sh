...
CONFIG_ROOT="${REPO_ROOT}/configs_exploration_CoordNet"
...
if [[ "${#CONFIGS[@]}" -ne "${EXPECTED_TOTAL}" ]]; then
    printf 'Expected %d exploration_CoordNet configs, found %d. Regenerate with scripts/generate_exploration_coordnet_configs.py.\n' \
...
if [[ "${DRY_RUN}" != "1" ]]; then
    ...
    if ! conda run --no-capture-output -n "${CONDA_ENV}" python \
        "${SCRIPT_DIR}/summarize_exploration_coordnet.py" "${summary_args[@]}"; then

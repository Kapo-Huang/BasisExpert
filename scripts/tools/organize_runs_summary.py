...
def _batch_config_parts(config_key: str) -> tuple[str, str, str]:
    ...
    if len(parts) > 2 and parts[0].lower() == "configs":
        category = parts[1].lower()
        model_index = 2 if category in {"main", "rd_curve"} else 3
    else:
        # Legacy layout: configs_exploration_vN/<model>/...
        model_index = 1 if len(parts) > 1 else 0
    model_index = min(model_index, max(len(parts) - 1, 0))

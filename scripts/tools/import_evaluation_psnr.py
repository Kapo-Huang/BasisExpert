#!/usr/bin/env python3
"""Import valid PSNR and perceptual measurements into ``Result`` reports.
...
sampled timesteps is preferred; recipe order is used as a deterministic
tie-break. Tied PSNR measurements must agree, preserving the existing import
contract. SSIM and LPIPS are always selected together from one evaluation.
"""
...
SUMMARY_MARKER_PREFIX = "> PSNR values marked as evaluation-imported"
SUMMARY_MARKER = (
    "> PSNR values marked as evaluation-imported and SSIM/LPIPS values were "
    "read from completed checkpoint evaluations with at least 10 uniformly "
    "selected timesteps. Missing metrics are shown as `-`."
)

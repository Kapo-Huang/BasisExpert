...
RUN_TOKEN="${RUN_TOKEN:-coordnet_${DATASET}_$(date +%Y%m%d_%H%M%S)}"
...
mkdir -p "${LOG_ROOT}"
{
    printf '# CoordNet %s main experiments.\n' "${DATASET}"
    for config in "${REPO_ROOT}/configs/main/CoordNet/${DATASET}"__*.yaml; do
        printf 'configs/main/CoordNet/%s\n' "${config##*/}"
    done
} > "${CONFIG_LIST_FILE}"

...
RUN_TOKEN="${RUN_TOKEN:-siren_${DATASET}_$(date +%Y%m%d_%H%M%S)}"
...
mkdir -p "${LOG_ROOT}"
{
    printf '# SIREN %s main experiments.\n' "${DATASET}"
    for config in "${REPO_ROOT}/configs/main/SIREN/${DATASET}"__*.yaml; do
        printf 'configs/main/SIREN/%s\n' "${config##*/}"
    done
} > "${CONFIG_LIST_FILE}"

...
RUN_TOKEN="${RUN_TOKEN:-moe_${DATASET}_$(date +%Y%m%d_%H%M%S)}"
...
mkdir -p "${LOG_ROOT}"
{
    printf '# MoE-INR %s main experiments.\n' "${DATASET}"
    for config in "${REPO_ROOT}/configs/main/MoE-INR/${DATASET}"__*.yaml; do
        printf 'configs/main/MoE-INR/%s\n' "${config##*/}"
    done
} > "${CONFIG_LIST_FILE}"

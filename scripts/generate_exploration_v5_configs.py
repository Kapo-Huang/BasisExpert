...
CONFIG_ROOT = ROOT / "configs_exploration_v5"
...
def generate_fv_srn() -> int:
    ...
    for target in TARGETS:
        source = _read(FORMAL_ROOT / "fV-SRN" / "Size163" / f"ionization__{target}.yaml")
...
def generate_instant_vnr() -> int:
    count = 0
    for target in TARGETS:
        source = _read(FORMAL_ROOT / "InstantVNR" / f"ionization__{target}.yaml")
